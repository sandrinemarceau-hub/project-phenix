...
# ==========================================
st.set_page_config(layout="wide", page_title="Portail Logistique V36")
st.title("📦 Portail de Disponibilité - VERSION 36 🔴")
st.write("Ciblage bi-directionnel : Lecture des colonnes ART SORTIE et ART ENTRÉE.")
...
if st.button("🚀 Calculer les disponibilités (V36)", type="primary", use_container_width=True):
    if fichier_stock and fichiers_prod and fichier_commandes:
        with st.spinner('Analyse, extraction et chaînage bi-directionnel...'):
            try:
                log_diagnostic = []
                dict_prepa = {}
                dict_details = {}
                ...
                # --- C. LECTURE PRODUCTION (PRIORITÉ SORTIE V36) ---
                liste_prod = []
                ...
                for f in fichiers_prod:
                    ...
                    # NOUVEAUTÉ V36 : On cherche d'abord les colonnes de SORTIE
                    liste_articles_prod = ['CODEARTSORTIE', 'ARTSORTIE', 'CODEARTENTREE', 'ARTENTREE', 'ARTICLECODEAE', 'ARTICLECODE', 'CODEARTICLE', 'ARTICLE', 'REFERENCE', 'CODE']
                    liste_qtes_prod = ['QTEARTSORTIE', 'QTESORTIE', 'QTEARTENTREE', 'QTEENTREE', 'QTEAE', 'RESTEAFAIRE', 'RESTE', 'AFAIRE', 'QTEPREVUE', 'QUANTITEPREVUE', 'QUANTITE']
                    ...
                    if col_art_prod and col_qte_prod:
                        ...
                        for col in colonnes_dates_potentielles:
                            ...
                            if not s_test.isna().all():
                                if date_series is None: date_series = s_test; col_date_utilisee = col
                                else: date_series = date_series.fillna(s_test); col_date_utilisee += f" + {col}"
                        ...
                        log_diagnostic.append(f"✅ **{f.name}** : Art=`{col_art_prod}`, Qté=`{col_qte_prod}`, Date=`{col_date_utilisee}`.")
                    else:
                        log_diagnostic.append(f"❌ **{f.name}** : Ignoré (Art/Qté non détecté).")
...
if st.session_state['calcul_ok']:
    ...
    with c_btn1:
        ...
        st.download_button("📥 Télécharger l'Excel", data=buffer, file_name="Analyse_V36.xlsx", type="primary")
    with c_btn2:
    ...
    st.divider()
    st.subheader("🕵️‍♂️ Scanner Global & Généalogie V36")
